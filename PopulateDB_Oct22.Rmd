...
dbname<-"synaptic.proteome_SR_ASD_EPS_20221010.db.sqlite"
recreate<-TRUE
```
...
ddlV5<-paste("CREATE VIEW AllMutationsAllPapers AS",
"SELECT      m.GeneID,",
"            MGI,",
"            MouseEntrez,",
"            MouseName,",
...
"            p.Name AS Paper",
"FROM  Gene g ",
"JOIN Mutation m ON g.ID = m.GeneID",
"JOIN Disease d ON m.HDOID = d.HDOID",
"JOIN PaperMutation pm ON pm.MutationID=m.ID",
"JOIN Paper p ON p.PMID=pm.PMID;")
```
...
mut.df <- mut.df[, c("ID","GeneID", "HDOID","Chr", "Position", "Variant","FunctionClass", "cDnaVariant", "ProteinVariant","Exon.Intron", "Denovo", "SFARI","EpilepsyGene", "ClinVar")]
names(mut.df) <- c("ID","GeneID", "HDOID","Chromosome", "Position", "Variant","FunctionClass", "cDNAvariant", "ProteinVariant","ExonIntron", "DENOVO", "SFARI","EpilepsyGene", "ClinVar")
